...
class InstanceGenerator(Configurable):
    ...
    def _process_images(self, seq_name, n_random_points=False, save_points=True, save_boxes=False,
                        save_masks=False, save_labels=False, save_instances_masks=False,
                        save_instances_boxes=True):
        ...
        if save_instances_boxes:
            rd_point_path = os.path.join(self.paths['carrada'], seq_name, 'rd_points.json')
            with open(rd_point_path, 'rb') as fp:
                self.rd_points = json.load(fp)           
        else:
            self.rd_points = dict()
        print('*** Processing Sequence:  %s ***' % seq_name)
        for img_path in img_paths:
            print('*** Processing Image:  %s ***' % img_path.split('/')[-1].split('.')[0])
            image = ImageInstances(img_path, self.model, self.paths,
                                   seq_mot_tracker, n_random_points,
                                   save_instances_masks,
                                   self.rd_points,
                                   save_instances_boxes)
    ...
    def process_sequence(self, seq_name, n_points=False, save_points=False, save_boxes=False,
                         save_masks=False, save_labels=False, save_instances_masks=False,
                         save_instances_boxes=True):
        """
...
class ImageInstances:
    ...
    def __init__(self, path_to_image, model, paths, mot_tracker, n_random_points=False,
                 save_instances_masks=False, rd_points=dict(), save_instances_boxes=True):
        ...
        self.save_instances_boxes = save_instances_boxes
        ...
        if save_instances_boxes:
            try:
                self.img_rd_points = rd_points[self.img_name]
            except KeyError:
                self.img_rd_points = dict()    
    ...
    def update_instances(self, get_rgb_masks=False):
        ...
        # zlw@20211018
        rgb_masks = list()
        if self.save_instances_boxes:
            instances_static = self.get_instance_static()
            _instances = list()
            _boxes = list()
            _labels = list()
            _rd_points = list()
        ...
        for i in range(self.labels.shape[0]):
            ...
            if self.save_instances_boxes:
                try:
                    _instances.append(instances_static[i])
                except IndexError:
                    _instances.append(instance_name)
                _boxes.append(self.boxes[i])
                _labels.append(self.labels[i])
                try:
                    _rd_points.append(self.img_rd_points[instances_static[i]])
                    #_rd_points.append(self.img_rd_points[instance_name])
                except IndexError:
                    _rd_points.append({"distance": [None], "velocity": [None]})
                except KeyError:
                    _rd_points.append({"distance": [None], "velocity": [None]})
        ...
        # zlw@20211018
        if self.save_instances_boxes:
            img_boxes_path = os.path.join(self.paths['carrada'], self.seq_name, 'img_boxes')
            os.makedirs(img_boxes_path, exist_ok=True)
            box_path = os.path.join(img_boxes_path, self.img_name + '.png')
            save_image_boxes(self.path_to_image, box_path,
                             _rd_points, _instances, _boxes, _labels)
        if get_rgb_masks:
            if rgb_masks:
    ...
    def get_instance_static(self):
        """Get instance name from static points.json file"""
        points_path = os.path.join(self.paths['carrada'], self.seq_name, 'points.json')
        with open(points_path, 'rb') as fp:
            points = json.load(fp)
        _instances = list(points[self.seq_name][self.img_name].keys())
        return _instances
...
# zlw@20211018
def save_image_boxes(img_path, path_to_save,
                     rd_points, instances, boxes, labels):
    """Save image with bboxes"""
    bbox_color = {1: "y", 2: "r", 3: "c"}
    tbox_color = {1: (1, 1, 0.8, 0.5),
                  2: (1, 0.5, 0.5, 0.5), 
                  3: (0.8, 1, 1, 0.5)}
    label_name = { 
            1: "Pedestrian",
    ...
    for i in range(len(instances)):
        bbox_w = boxes[i][2] - boxes[i][0]
        bbox_h = boxes[i][3] - boxes[i][1]
        bbox = patches.Rectangle((boxes[i][0], boxes[i][1]),
                                 bbox_w, bbox_h,
                                 linewidth=2,
                                 edgecolor=bbox_color[labels[i]],
                                 facecolor='none')
        ax.add_patch(bbox)
        try:
            info = 'Obj_ID: #%s\nObj_Type: %s\nDistance: %f(m)\nVelocity: %f(m/s)'\
                        %(instances[i], label_name[labels[i]], \
                          rd_points[i]["distance"][0], rd_points[i]["velocity"][0])
            print("distance:", rd_points[i]["distance"])
        except TypeError:
            info = 'Obj_ID: #%s\nObj_Type: %s\nDistance: %s(m)\nVelocity: %s(m/s)'\
                        %(instances[i], label_name[labels[i]], "None", "None")
        ax.annotate(info, 
                    (boxes[i][0], boxes[i][1]),
                    xytext=(5, 5),
                    ...
                    bbox=dict(boxstyle="square",
                              fc=tbox_color[labels[i]],
                              ec='grey',
                              linestyle='--'))
        # plt.text(self.box[0], self.box[1], info,
                 # color=text_color[self.label],
                 # verticalalignment='bottom')
    plt.savefig(path_to_save, bbox_inches='tight', pad_inches=0.0)
    plt.close()
