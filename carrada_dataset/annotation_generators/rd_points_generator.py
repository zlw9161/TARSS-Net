...
from .rdr_utils import gen_rd_proposals, rd_coord_calibration,gen_rd_proposals_cfar,rd_coord_calibration_zxy
...
class RDPointsGenerator(Configurable):
    ...
    # zlw@20211012
    def _compute_rd_points_coordinates(self, rd_instance_points, doppler_shape):
        """Convert estimated distance and velocity to RD resolution"""
        rd_points_coordinates = dict()
        rd_points_coordinates['range'] = [int(rd_instance_points['distance'][i]/ \
                                              self.cls.RANGE_RES)
                                          for i in range(self.n_points)]
        rd_points_coordinates['doppler'] = [int(doppler_shape/2 -1) +
                                            int(rd_instance_points['velocity'][i]/ \
                                                self.cls.DOPPLER_RES)
                                            for i in range(self.n_points)]
        rd_points_coordinates['distance'] = [rd_instance_points['distance'][i]
                                             for i in range(self.n_points)]
        rd_points_coordinates['velocity'] = [rd_instance_points['velocity'][i]
                                             for i in range(self.n_points)]
        return rd_points_coordinates

 # zlw@20211206
    def _calibrating_rd_points(self, rd_points_coordinates, rd_matrix, rd_id, instance):
        """calibrating the rd points using filtered rd-image for each instance"""
        clb_rd_points = dict()
        rd_proposals, zero_dop_ax, rd_matrix_thf = gen_rd_proposals_cfar(rd_matrix)
        for i in range(self.n_points):
            range_coord = rd_points_coordinates[rd_id][instance]['range'][i]
            doppler_coord = rd_points_coordinates[rd_id][instance]['doppler'][i]
            clb_coord, clb_range, clb_velocity = rd_coord_calibration_zxy(rd_proposals,
                                                                      rd_matrix_thf,
                                                                      zero_dop_ax,
                                                                      range_coord,
                                                                      doppler_coord)
            clb_rd_points['range'] = clb_coord[0]
            clb_rd_points['doppler'] = clb_coord[1]
            clb_rd_points['distance'] = clb_range
            clb_rd_points['velocity'] = clb_velocity
        return clb_rd_points

    # zlw@20211013
    def _save_rd_point_images(self, rd_points_coordinates, rd_matrix, rd_id):
        """Save RD images with projected estimated points """
        annotated_rd_path = os.path.join(self.paths['seq_data'], 'rd_calibrated_img_boxes')
            ...
            for idx, instance in enumerate(instances):
                ...
                for i in range(self.n_points):
                    ...
                    clb_coord, clb_range, clb_velocity = rd_coord_calibration_zxy(rd_proposals,
                                                                              ...
                                                                              range_coord,
                                                                              doppler_coord)
                    rd_coord.append(clb_coord)
                    dist = clb_range
                    velo = clb_velocity
                    label = self.labels[self.seq_name][rd_id][instance]
                    rd_instance_info.append([dist, velo, instance, label])
                visualiser.add_annotation(idx, np.array(rd_coord), 'sparse')
                # @zlw20211011
    ...
    def get_rd_points(self, save_rd_imgs=False, save_points=False,
                      save_clb_points=False,
        ...
        for i in range(self.time_window+1, self.seq_len):
            ...
            for instance in self.instances:
                ...
                try:
                    ...
                    clb_rd_points[rd_id_t2][instance] = self._calibrating_rd_points(rd_points_coordinates,
                                                                                    rd_matrix_t2, 
                                                                                    rd_id_t2, 
                                                                                    instance)
                except KeyError:
        ...
        if save_clb_points:
            self._save_points(clb_rd_points, 'clb_rd_points.json')
