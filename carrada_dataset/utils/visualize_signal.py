...
class SignalVisualizer():
    ...
    # zlw@20211012
    def save_multiple_annotations(self, path, instance_coord, instance_info,
                                  signal_type='range_doppler', color_scale=None,
                                  rotation=False, save_img=True, plot_img=False):
        """
        ...
        img = self._format_img(transformed_image, signal_type, color_scale, 
                               rotation, instance_coord, instance_info)
        if save_img:
            plt.savefig(path)
        if plot_img:
            # zlw@20211011
    ...
    def _format_img(self, img, signal_type, color_scale, rotation,
                    instance_coord, instance_info, showinfo=True):
        ...
        if signal_type == 'range_doppler':
            ...
            ax.set_xlabel('Doppler Effect', fontsize=20)
            # zlw@20211013
            if showinfo:
                self.mark_rd_point(ax, instance_coord, instance_info)
        else:
            ...
            # zxy@20220612
            if showinfo:
                self.mark_ra_point(ax, instance_coord, instance_info)
        if rotation:
        ...
        if color_scale:
            ...
            cbar.ax.set_ylabel('Normalised Intensity', fontsize=20)
    ...
    # zlw@20211013
    def mark_rd_point(self, ax, instance_rd_coord, instance_rd_info):
        """"Markdown the instance points in rd image"""
        mk_cl = {"1": "y", "2": "r", "3": "c"}
        mk_sp = {"1": "*", "2": "^", "3": "h"}
        mk_sz = {"1": 60, "2": 60, "3": 80}
        txt_bx_cl = {"1": (1, 1, 0.8, 0.75),
                     "2": (1, 0.5, 0.5, 0.75), 
                     "3": (0.8, 1, 1, 0.75)}
        cnctstyle = {0: "angle,angleA=60,angleB=0,rad=0",
                     1: "angle,angleA=60,angleB=0,rad=0"}
        #print("marker_num: ", len(instance_rd_coord))
        for n in range(len(instance_rd_coord)):
            pt_x = instance_rd_coord[n][1]*self.scaling_factor
            pt_y = instance_rd_coord[n][0]*self.scaling_factor
            dst = instance_rd_info[n][0][0:6]
            vel = instance_rd_info[n][1][0:6]
            ins = instance_rd_info[n][2]
            lab = self.label_transform(instance_rd_info[n][3])
            
            ax.scatter(pt_x, pt_y,
                       s=mk_sz[instance_rd_info[n][3]],
                       c=mk_cl[instance_rd_info[n][3]], 
                       marker=mk_sp[instance_rd_info[n][3]])
            info = 'Obj_ID: #%s\nObj_Type: %s\nDistance: %s(m)\nVelocity: %s(m/s)' %(ins,lab,dst,vel)
            ax.annotate(info, 
                        (pt_x, pt_y),
                        xytext=(40, (n%2-1)*15+55),
                        textcoords='offset points',
                        arrowprops=dict(arrowstyle="-|>",
                                        connectionstyle=cnctstyle[n%2],
                                        facecolor='k',
                                        ec='k'),
                        fontsize=12,
                        bbox=dict(boxstyle="square",
                                  fc=txt_bx_cl[instance_rd_info[n][3]],
                                  ec='k',
                                  linestyle='--'))

    def mark_ra_point(self, ax, instance_ra_coord, instance_ra_info):
        """"Markdown the instance points in ra image"""
        mk_cl = {"0": "b", "1": "y", "2": "r", "3": "c"}
        mk_sp = {"0": "8", "1": "*", "2": "^", "3": "h"}
        mk_sz = {"0": 60, "1": 60, "2": 60, "3": 80}
        txt_bx_cl = {"0": (1, 1, 0.8, 0.75),
                     "1": (1, 1, 0.8, 0.75),
                     "2": (1, 0.5, 0.5, 0.75),
                     "3": (0.8, 1, 1, 0.75)}
        cnctstyle = {0: "angle,angleA=60,angleB=0,rad=0",
                     1: "angle,angleA=60,angleB=0,rad=0"}
        # print("marker_num: ", len(instance_rd_coord))
        for n in range(len(instance_ra_coord)):
            pt_x = instance_ra_coord[n][1] * self.scaling_factor
            pt_y = instance_ra_coord[n][0] * self.scaling_factor
            dst = instance_ra_info[n][0][0:6]
            angle = instance_ra_info[n][1][0:6]
            ins = instance_ra_info[n][2]
            lab = self.label_transform(instance_ra_info[n][3])

            ax.scatter(pt_x, pt_y,
                       s=mk_sz[instance_ra_info[n][3]],
                       c=mk_cl[instance_ra_info[n][3]],
                       marker=mk_sp[instance_ra_info[n][3]])
            info = 'Obj_ID: #%s\nObj_Type: %s\nDistance: %s(m)\nAngle: %s(°)' % (ins, lab, dst, angle)
            ax.annotate(info,
                        (pt_x, pt_y),
                        xytext=(40, (n % 2 - 1) * 15 + 55),
                        textcoords='offset points',
                        ...
                        fontsize=12,
                        bbox=dict(boxstyle="square",
                                  fc=txt_bx_cl[instance_ra_info[n][3]],
                                  ec='k',
                                  linestyle='--'))
    ...
    def label_transform(self, label):
        """Transform the label to its real name"""
        label_name = { 
            "1": "Pedestrian",
            "2": "Cyclist",
            "3": "vehicle"
            }
        return label_name.get(label, None)
