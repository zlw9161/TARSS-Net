...
#### Spatial Squeeze w/ GAP w/ PosEnc (Attention is all you need) ####
#### TAM-ASPP Fusion & Latent Space
#### ATT-Pool: weighted summation + current frame nn.Functional.softmax --> nn.softmax;
...
class ATTPool(nn.Module):
    ...
    def forward(self, x, x_tre):
        ...
        att_mat = self.fc_layer(x_tre)
        # generate time-wise attention scores
        att_sc = self.att_scoring(att_mat)
...
class TAM(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, signal_type):
        ...
        '''self.tr_embedding_3 = TREBlock(in_ch*2, out_ch*2, 
                                       ...
                                       strd=(1, 2, 2),
                                       pad=(0, 0, 0))'''
        self.spatial_squeeze = nn.AvgPool3d(kernel_size=(1, 16, 16),
                                            stride=(1, 16, 16))
    ...
    def forward(self, x):
        ...
        # print("x2: ", x2.shape)
        x2_spsq = self.spatial_squeeze(x2)
